...
use gen::*;
use gen::display_control_server::DisplayControlServer as GenDisplayControlServer;
...
impl display_control_server::DisplayControl for DisplayServer {
    ...
    type AttachStream = ReceiverStream<Result<AttachEvent, Status>>;

    async fn attach(&self, _req: Request<AttachRequest>) -> Result<Response<Self::AttachStream>, Status> {
        ...
        tokio::spawn(async move {
            ...
            let stream = match listener.accept().await {
                ...
                Err(err) => {
                    ...
                    send_or_log(&tx, Err(status)).await;
                    return;
                }
            };
            ...
            tokio::select! {
                ...
                result = stream_displayer.display(display_info, stream) => {
                    match result {
                        Ok(()) => (),
                        Err(err) => send_or_log(&tx, Err(err)).await,
                    }
                }
            }
        });
...
async fn send_or_log<T>(tx: &mpsc::Sender<T>, msg: T) {
    match tx.send(msg).await {
        Ok(()) => (),
        Err(_) => {
            warn!("Failed to send to channel {:?}", tx)
        }
    }
}
